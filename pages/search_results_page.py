...
class SearchResultsPage:
    ...
    def apply_filters(self, filter_data: dict):
        for group, value in filter_data.items():
            if group == "Your budget (per night)":
                slider_width = int(self.webdriver_ops.execute_js_script_on_element(
                    'return arguments[0].scrollWidth', price_slider_bar, 'Slider bar'))
                slider_min_value = int(self.webdriver_ops.execute_js_script_on_element(
                ...
                self.webdriver_ops.click(
                    filter_group, 'Select Filter', [group, value])
                self.webdriver_ops.wait_for_element_to_be_visible(
                    filter_tag, "Filter Tag", value)
            self.webdriver_ops.wait_for_element_to_be_visible(
                data_testid_locator, "Property card", "property-card")
    ...
    def verify_properties_for_applied_filter(self, request_data, filter_data: dict):
        for i in range(1, self.webdriver_ops.get_number_of_elements(
                data_testid_locator, "Property card",  "property-card") + 1):
            for group, value in filter_data.items():
                locator, value = self.get_locator_for_property_dtl(
                    group, value)
                if group == "Your budget (per night)":
                    self.webdriver_ops.is_element_present(
                        propery_card_duration_member_info, "Duration Memeber Info",  stop_on_fail=True,
                    ...
                    self.webdriver_ops.is_element_present(
                        locator,  f"Property card with {group}",  stop_on_fail=True, replace_value=[i, value])
